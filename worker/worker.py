...
except Exception:  # pragma: no cover - optional at import time for CI syntax checks
    Client = None
    raw = None
...
VOICE_WHEEL_URL = os.getenv(
    "VOICE_WHEEL_URL",
    "https://hoseinsoltanzade1985-hue.github.io/Simorgh/games/voice-wheel.html",
).strip()
...
class Store:
        ...
        request(
            ...
            json={
                ...
                "details": {
                    "version": "4.7.1",
                    "mode": "voice-only",
                    "roster": "current-at-execution",
                    "wheel": "multi-winner-elimination",
                },
    ...
    def group_chat_id(self, group_id: int) -> int:
        """Resolve the internal groups.id FK to the real Telegram chat id.

        Critical: voice MTProto APIs require the Telegram chat id, while the queue
        stores the database group's primary key.
        """
        response = request(
            "GET",
    ...
    def snapshot(self, group_id: int, people: list[dict]) -> None:
        ...
        if rows:
            request(
                "POST",
                f"{self.base}/voice_call_presence_snapshots",
...
async def notify(chat_id: int, people: list[dict], count: int) -> None:
    ...
    payload = {
        ...
        "reply_markup": {
            "inline_keyboard": [[
                {"text": "🎡 اجرای گردونه", "web_app": {"url": wheel_url}}
            ]]
        },
    }
    request(
                ...
                try:
                    ...
                    store.complete(
                        ...
                        "DONE",
                        {
                            ...
                            "effective_count": chosen_count,
                            "count": len(people),
                            "users": result_users,
                            "wheel": "multi-winner-elimination",
                        },
                    )
                except Exception as exc:  # pragma: no cover - production network path
